...
//  0.1.04  2013-10-17  added getAverage(uint16_t) - kudo's to Sembazuru
...
RunningMedian::RunningMedian(const uint16_t size)
{
  ...
  _sortIdx  = (uint16_t *) malloc(_size * sizeof(uint16_t));
#endif
  clear();
}
...
float RunningMedian::getQuantile(float q)
{
  ...
  const uint16_t lo = floor(id);
  const uint16_t hi = ceil(id);
...
float RunningMedian::getAverage(uint16_t nMedians)
{
  if ((_count == 0) || (nMedians == 0)) return NAN;

  if (_count < nMedians) nMedians = _count;     // when filling the array for first time
  uint16_t start = ((_count - nMedians) / 2);
  uint16_t stop = start + nMedians;
...
float RunningMedian::getElement(const uint16_t n)
{
  if ((_count == 0) || (n >= _count)) return NAN;

  uint16_t pos = _index + n;
...
float RunningMedian::getSortedElement(const uint16_t n)
{
  if ((_count == 0) || (n >= _count)) return NAN;
...
float RunningMedian::predict(const uint16_t n)
{
  uint16_t mid = _count / 2;
