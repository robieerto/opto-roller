...
// max 250 to not overflow uint8_t
...
public:
  ...
  explicit RunningMedian(const uint16_t size);
  ~RunningMedian();
  ...
  float   getAverage(uint16_t nMedian);
  ...
  float   getElement(const uint16_t n);
  // get n'th element from the values in size order
  float   getSortedElement(const uint16_t n);
  // predict the max change of median after n additions
  float   predict(const uint16_t n);
...
protected:
  ...
  uint16_t   _size;      // max number of values
  uint16_t   _count;     // current number of values
  uint16_t   _index;     // next index to add.
